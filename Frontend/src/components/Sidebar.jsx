...
const Sidebar = ({ onCreatePlannerClick }) => {
  ...
  return (
    <aside className={`bg-white shadow-lg min-h-screen w-64 fixed top-0 left-0 z-30 transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
      {/* Logo */}
      ...
      <nav className="flex flex-col gap-4 p-4">
        ...
        <Link to="/dashboard" className={`flex items-center gap-2 text-gray-700 hover:text-indigo-600 ${isActive("/dashboard") ? "text-indigo-600" : ""}`}>
          <i className={`fas fa-chart-line mr-3 ${isActive("/dashboard") ? "text-indigo-600" : "text-gray-400"}`}></i> Dashboard
        </Link>
        <Link to="/yearly" className={`flex items-center gap-2 text-gray-700 hover:text-indigo-600 ${isActive("/yearly") ? "text-indigo-600" : ""}`}>
          <i
              className={`fas fa-calendar-alt mr-3 ${isActive("/yearly") ? "text-indigo-600" : "text-gray-400"}`}
            ></i> Yearly Planner
        </Link>
        <Link to="/monthly" className={`flex items-center gap-2 text-gray-700 hover:text-indigo-600 ${isActive("/monthly") ? "text-indigo-600" : ""}`}>
          <i
              className={`fas fa-calendar-day mr-3 ${isActive("/monthly") ? "text-indigo-600" : "text-gray-400"}`}
            ></i> Monthly Planner
        </Link>
        <Link to="/weekly" className={`flex items-center gap-2 text-gray-700 hover:text-indigo-600 ${isActive("/weekly") ? "text-indigo-600" : ""}`}>
          <i
              className={`fas fa-calendar-week mr-3 ${isActive("/weekly") ? "text-indigo-600" : "text-gray-400"}`}
            ></i> Weekly Planner
        </Link>
